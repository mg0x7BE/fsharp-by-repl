...
Learn F# from scratch by solving exercises in the REPL. When you finish the ones
here, point Claude at [`CLAUDE.md`](CLAUDE.md) and it generates the next one in
the same style.

All exercises so far were generated by Claude and reviewed by me.
...
3. In any `.fsx` file, select a code block and press `Ctrl + \`

> If `Ctrl + \` doesn't work: Settings → Keymap → Main Menu → Tools →
> F# Interactive → *Send to F# Interactive*.
...
**VS Code + [Ionide](https://ionide.io/):** select code, press `Alt + Enter`.
